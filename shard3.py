...
def load_tfrecord_x_y_pair_recon_dataset(tfrecord_dir): 
    ...
    file_path_dataset = tf.data.Dataset.from_tensor_slices(sorted_file_list)
        #^^^ file list dataset
...
def configure_for_performance(dataset, bufferSize, batchSize):
    ...
    return dataset
def fltbatch_plt(fltBatch, fltBatchSize, Name, ImageOutDir):
    for i in range(fltBatchSize):
        fltname = f'{ImageOutDir}/{Name}_{i}.png'
