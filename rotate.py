...
def poisson_noise_to_normalized_flt(fltTensor):
	poissonTensor = tf.random.poisson(lam=0.0025, shape=tf.shape(fltTensor), dtype=tf.float32, seed=29)
...
def random_uniform_brightness_shift(fltTensor):
	ur_delta = tf.random.uniform([], minval=-0.06, maxval=0.06, dtype=tf.float32, seed=14)
...
adjusted_brightness_x = tf.vectorized_map(random_uniform_brightness_shift, flt_rfull_x)
